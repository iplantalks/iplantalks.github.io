...
const TableRow = ({
  ...
  return (
    <tr className='border-t border-neutral-200'>
      ...
      <td className="p-2">{currency(Math.min(...data.map((x) => x.value)))}%</td>
      <td className="p-2">{currency(data.map((x) => x.value).reduce((acc, x) => acc + x, 0) / data.length)}%</td>
      <td className="p-2">{currency(Math.max(...data.map((x) => x.value)))}%</td>
      <td className="p-2">
        {currency(
          data
      ...
      <td className="p-2">{currency(data[data.length - 1].value)}%</td>
      <td className="p-2">
        {!found && (
          <button className="block w-full px-2 py-1 text-sm rounded bg-blue-600 text-white font-semibold shadow hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition" onClick={() => toggle(id)}>
            додати
          </button>
        )}
        {found && (
          <button className="block w-full px-2 py-1 text-sm rounded bg-red-600 text-white font-semibold shadow hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 transition" onClick={() => toggle(id)}>
            прибрати
          </button>
        )}
      </td>
...
const TableRowComminSoon = ({ title, money }: { title: string; money: string }) => {
  return (
    <tr>
      <td className="p-2 text-neutral-500">{title}</td>
      <td className="p-2 text-neutral-500">{money}</td>
  ...
  return (
    <main>
      ...
      <div className="container mx-auto my-5 p-4">
        <h1 className='text-2xl font-bold mb-3'>Ураїнскій Ринок 🇺🇦</h1>
        <h2 className='text-xl font-bold mb-3'>Гривня через 20 років або Прибуткові ігри*</h2>
        <p className="mb-3">З колегами з iPlan.ua та Діловий клуб «Партнер» ми давно виношували ідею порівняти прибутковість інвестиційних інструментів, доступних українцям, на якомога довгому тимчасовому горизонті 15-20 років. Це цілком підходящий нормальний період для довгострокових накопичень чи то пенсія, чи весілля навчання дитини за кордоном.</p>
        <p className="mb-3">Ми вибрали ті інвестиційні інструменти, які вважають доступними будь-кому, хто намагається примножити свої заощадження, не вдаючись до ризикованих кроків:</p>
        ...
        <p className="mb-3">Дополнительно для сравнения была взята Инфляция, чтобы понимать какой из инструментов реально защитил Ваши сбережения от обесценивания.</p>
        <h3 className='text-xl font-bold mb-3'>Стартовые параметры инвестирования: сумма – 10000 грн, январь 1998, 2008, 2013 гг.</h3>
        <p className="mb-3"><b>Порядок виміру/усереднення курсів/ставок:</b></p>
        <ul className='list-disc list-inside ml-5 my-3'>
        ...
        <p className="mb-3"><b>Припущення:</b></p>
        <ul className='list-disc list-inside ml-5 my-3'>
        ...
        <p className="mb-3"><b>Джерела інформації:</b></p>
        <ul className='list-disc list-inside ml-5 my-3'>
        ...
        <p className="mb-3"><b>Проблеми проведення аналізу:</b></p>
        <ul className='list-disc list-inside ml-5 my-3'>
        ...
        <p className="mb-3">Для чистоти експерименту ми вирішили провести аналіз за 3 різні періоди, так щоб кожен із періодів захопив хоча б одну кризу 1998, 2008 та 2014 років. Це дозволить протестувати на стійкість різноманітні сценарії інвестування.</p>

        <h3 className='text-xl font-bold mb-3'>За підсумками аналізу, трійка лідерів найприбутковіших інвестиційних інструментів виглядає так:</h3>

        <ol className='list-decimal list-inside ml-5 my-3'>
          <li>
            <b>За период 1998-2018г</b>
            <ol className='list-decimal list-inside ml-5 my-3' type="a">
              <li>Недвижимость в Киеве для сдачи в аренду</li>
              <li>Депозит в доларе</li>
        ...
        <p className="mb-3"><b>Короткий опис реалізованих ризиків/можливостей за період аналізу:</b></p>
        ...
        <h3 className='text-xl font-bold mb-3'>Выводы анализа:</h3>
        ...
        <p className="mb-3">Було усе це пораховано і опубліковано аж у 2018 році, і якщо тобі цікава ця тема, читай далі та спробуй змоделювати свою історію.</p>

        <h2 className="text-2xl font-bold my-5">
          <span className="text-neutral-500">Крок перший -</span> оберімо активи 🔎
        </h2>
        <p className="mb-3">
        ...
        <table className="table-auto w-full text-center">
          <thead>
            <tr>
              ...
              <th className="p-2">min(inception)</th>
              <th className="p-2">avg(inception)</th>
              <th className="p-2">max(inception)</th>
              <th className="p-2">avg(10y)</th>
              <th className="p-2">avg(5y)</th>
        ...
        <h2 className="text-2xl font-bold my-5">
          <span className="text-neutral-500">Крок другий -</span> розпреділимо активи 📊
        </h2>
        <p className="mb-3">Ваша мета - розпреділити активи таким чином щоб мати більш менш очікувану дохідність. Також варто спробувати подивитися результати на різних проміжках часу за для більшої впеноності.</p>
        <p className="mb-3">За для зручності додано графік інфляції - в ідеалі портфель має обганяти її.</p>
              ...
              <tbody>
                {allocations.map(({ id, value, locked }, idx) => (
                  <tr key={id}>
                    ...
                    <td className="p-1">
                      <input
                        className="px-2 py-1 text-sm border border-gray-300 rounded bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                        type="number"
                        min={0}
                        max={100}
                        step={1}
              ...
              <tbody>
                ...
                <tr>
                  <td className='p-1' align="left" valign="middle">
                    <input className="px-2 py-1 text-sm border border-gray-300 rounded bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" type="number" min={minDate} max={maxDate} value={startDate} onChange={(event) => setStartDate(event.target.valueAsNumber)} />
                  </td>
                  ...
                  <td className='p-1' align="right" valign="middle">
                    <input className="px-2 py-1 text-sm border border-gray-300 rounded bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" type="number" min={minDate} max={maxDate} value={endDate} onChange={(event) => setEndDate(event.target.valueAsNumber)} />
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div className="my-5">
          <div className="flex gap-4 items-center">
            <div className="flex gap-2 items-center">
              <input type="checkbox" checked={showInflation} onChange={() => setShowInflation(!showInflation)} id="showInflation" />
              <label htmlFor="showInflation">
                inflation
              </label>
            </div>
            <div className="flex gap-2 items-center">
              <input type="checkbox" checked={showActives} onChange={() => setShowActives(!showActives)} id="showActives" />
              <label htmlFor="showActives">
                actives
              </label>
            </div>
          </div>
