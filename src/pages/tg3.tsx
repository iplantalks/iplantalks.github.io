...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.telegramCallback = (user: TelegramUser) => {
      ...
      fetch('https://europe-west3-iplantalks.cloudfunctions.net/telegramdemo2', { method: 'POST', headers: { 'content-type': 'application/json; charset=utf-8' }, body: JSON.stringify(user) }).then(
        (r) => {
          r.text().then((text) => {
            if (r.status === 200) {
              user.status = text
